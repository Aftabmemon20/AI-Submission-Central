...
export default function JudgePortal() {
  ...
  // Jab user ID load ho jaaye, uske hackathons fetch karo
  useEffect(() => {
    if (isLoaded && userId) {
      setIsLoading(true);
      fetch(`http://localhost:5000/api/hackathons?judge_id=${userId}`)
        .then(res => res.json())
  ...
  // Naya hackathon create karne ka function
  const handleCreate = async () => {
    ...
    try {
        ...
        body: JSON.stringify({ name: newHackathonName, judge_id: userId, criteria: newHackathonCriteria }),
      });
      const data = await response.json();
      if (!response.ok) throw new Error(data.error || 'Failed to create hackathon');

      setCreatedHackId(data.hackathon.id);
      setHackathons([data.hackathon, ...hackathons]); // List mein naya hackathon add karo
      setNewHackathonName("");
        ...
        <Card className="bg-neutral-900 border-neutral-700">
          ...
          <CardContent className="space-y-4">
            <div className="space-y-2">
              ...
              <Input id="hack-name" value={newHackathonName} onChange={e => setNewHackathonName(e.target.value)} placeholder="e.g. Winter Codefest 2025" disabled={isLoading} />
            </div>
            <div className="space-y-2">
              <Label htmlFor="hack-criteria">Judging Criteria</Label>
              <Textarea id="hack-criteria" value={newHackathonCriteria} onChange={e => setNewHackathonCriteria(e.target.value)} rows={5} disabled={isLoading} />
            </div>
            <Button onClick={handleCreate} className="w-full" disabled={isLoading}>{isLoading ? 'Creating...' : 'Create New Hack ID'}</Button>
            {createdHackId && (
        ...
        <Card className="bg-neutral-900 border-neutral-700">
          ...
          <CardContent>
            {isLoading ? <p>Loading your hackathons...</p> : (
                ...
                {hackathons.map(h => (
                    <li key={h.id}>
                    <Link href={`/dashboard/${h.id}`} className="block p-4 rounded-md hover:bg-neutral-800 border border-neutral-700 transition-colors">
                        {h.name} (ID: {h.id})
                    </Link>
                    </li>
                ))}
